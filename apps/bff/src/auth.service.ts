...
type UserRecord = { id:string; username:string; displayName:string; role:Role; isAdmin:boolean; passwordHash:string; disabled?:boolean; teamId?:string; teamName?:string }
...
export class AuthService {
 ...
 private issue(user:UserRecord){if(this.secret().length<32)throw new UnauthorizedException('服务端会话密钥未配置。');const head=Buffer.from(JSON.stringify({alg:'HS256',typ:'JWT'})).toString('base64url'),payload=Buffer.from(JSON.stringify({sub:user.id,iat:Math.floor(Date.now()/1000),exp:Math.floor(Date.now()/1000)+28800})).toString('base64url');return head+'.'+payload+'.'+crypto.createHmac('sha256',this.secret()).update(head+'.'+payload).digest('base64url')}
  ...
  async saveUser(input: { id?: string; username?: string; displayName?: string; role?: Role; isAdmin?: boolean; password?: string; disabled?: boolean; teamId?: string; teamName?: string }) {
    ...
    if (!username || !displayName || !input.role || !validRoles.has(input.role)) throw new UnauthorizedException('账号信息或岗位无效。')
    const teamId = input.teamId?.trim() || undefined
    const teamName = input.teamName?.trim() || undefined
    if ((teamId && !teamName) || (!teamId && teamName)) throw new UnauthorizedException('销售小组 ID 和名称必须同时填写。')
    ...
    if (user) {
      ...
      user.username = username; user.displayName = displayName; user.role = input.role; user.isAdmin = input.isAdmin === true; user.disabled = input.disabled === true; user.teamId = teamId; user.teamName = teamName
      ...
      user = { id: crypto.randomUUID(), username, displayName, role: input.role, isAdmin: input.isAdmin === true, disabled: input.disabled === true, teamId, teamName, passwordHash: hash(input.password) }
